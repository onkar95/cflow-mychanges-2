...
function Steel({ getCart, theme, formData, modalopen, setModalOpen, newRequest, setNewRequest, data, setData, quantity, setQuantity, setOpenSaved, dropdownData }) {
    const [userId, setUserId] = useState(JSON.parse(localStorage.getItem('profile'))?.data?.id)
    const history = useHistory()
    const [selectedtype, setSelectedType] = useState("")

    const types = dropdownData?.steel_marks;
    ...
    const [placeholder1, setPlaceholder1]=useState("Steel marks")
    ...
    return (
        ...
        <div className="selected-header">Steel</div>
        <div className="description">Select steel type</div>
    ...
    { more===false &&
       <div>
        ...
        renderInput={params => (
          <TextField  placeholder={placeholder1} onFocus={()=>{setPlaceholder1("Search steel mark")}} onBlur={()=>{setPlaceholder1("Steel marks")}} {...params}  variant="outlined"  classes={{ root: theme?classes.inputtheme:classes.input }} />
        )}
      />
    ...
    <div style={{marginTop:'9.5%', marginBottom:'5%'}}>Didn't find your mark? <span class="add-more" onClick={()=>{setMore(true)}}>Click here to add</span></div>
     ...
     {more===true &&
        ...
        <TextField type="text" value={selectedtype} style={{ backgroundColor: theme?"#D8D8D8":"#08090C", width: "100%", height: "70%", borderRadius: "10px", color: "white" }} onChange={(e) => setSelectedType(e.target.value)} name="BrandInput" className={`${classes.root} InputField1`} InputProps={{ className: theme?classes.inputtheme:classes.input }} placeholder="Steel mark" variant="outlined" />
        </div>
